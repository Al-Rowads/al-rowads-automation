...
def test_replacement_preserves_matching_state_and_file_order(client):
    ...
    assert by_phone["+989121234567"]["completed"] is True
    assert by_phone["+989121234567"]["id"] == completed_id
    assert by_phone["+12025550101"]["completed"] is False
    assert "+971501234567" not in by_phone


def test_commit_rejects_stale_preview_without_changing_list(client):
    first = b"+989121234567\n"
    second = b"+971501234567\n"
    ...
    assert [contact["phone"] for contact in contacts] == ["+989121234567"]


def test_commit_rejects_file_changed_after_preview(client):
    preview = preview_file(client, b"+989121234567\n").get_json()

    response = commit_file(client, b"+971501234567\n", preview)
...
def test_upload_requires_txt_and_enforces_byte_limit(app, client):
    wrong_type = preview_file(client, b"+989121234567\n", "numbers.csv")
    ...
    oversized = client.post(
        "/api/imports/preview",
        data={"file": (io.BytesIO(b"+989121234567\n"), "numbers.txt")},
